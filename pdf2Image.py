...
def images_merge(images, images_number_in_one=2):
    ...
    for page_index in range(pages_number):
        # 新建空画布
        im = Image.open(io.BytesIO(images[0])) # 临时打开第一个图像获取尺寸
        pages_images.append(Image.new('RGB', (im.size[0]*images_number_in_one + ONE_CM_PIXEL*3, im.size[1] + ONE_CM_PIXEL*2), color='White'))
        for little_page_index in range(images_number_in_one):
            ...
            ImageDraw.Draw(im).line(
                [
                    (0+LINE_PIXEL, 0+LINE_PIXEL), (im.size[0]-LINE_PIXEL, 0+LINE_PIXEL),
                    (0+LINE_PIXEL, 0+LINE_PIXEL), (0+LINE_PIXEL, im.size[1]-LINE_PIXEL),
                    (0+LINE_PIXEL, im.size[1]-LINE_PIXEL), (im.size[0]-LINE_PIXEL, im.size[1]-LINE_PIXEL),
                    (im.size[0]-LINE_PIXEL, im.size[1]-LINE_PIXEL), (im.size[0]-LINE_PIXEL, 0+LINE_PIXEL)
                ],
                fill='Black', width=LINE_PIXEL
                )
            # 图像放到空画布中
            pages_images[page_index].paste(im, (
                ONE_CM_PIXEL*(little_page_index+1) + im.size[0]*little_page_index, 
                ONE_CM_PIXEL, 
                ONE_CM_PIXEL*(little_page_index+1) + im.size[0]*(little_page_index+1), 
                ONE_CM_PIXEL+im.size[1]
                ))
